...
1. Read the app-local `AGENTS.md` before editing inside `arena/`, `api/`, or `klinikk.no/`.
2. Do not duplicate shared UI, types, or domain logic across apps — move them to `packages/`.
3. Do not couple `arena/` and `klinikk.no/` directly. They share via `packages/ui` and via the API.
...
- `arena/` — authenticated workflow SPA (the workshop target)
- `klinikk.no/` — public marketing site, SEO-oriented, no patient data
